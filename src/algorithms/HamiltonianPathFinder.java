            ...
            switch(curCell.getCellType()) {
                case 'A':
                    ...
                    while (curCell.getY() + plusY > Map.maxY - 2 && plusY >= 3)
                        plusY--;
                    coorList.add(new DirectedCoordinate(curCell.getY() + plusY, curCell.getX(), DirectedCoordinate.SOUTH));
                    break;
                case 'B':
                    int plusX = 4;
                    while (curCell.getX() + plusX > Map.maxX - 2 && plusX >= 3)
                        plusX--;
                    coorList.add(new DirectedCoordinate(curCell.getY(), curCell.getX() + plusX, DirectedCoordinate.WEST));
                    break;
                case 'C':
                    int minusY = 4;
                    while (curCell.getY() - minusY < 1 && minusY >= 3)
                        minusY++;
                        coorList.add(new DirectedCoordinate(curCell.getY() - minusY, curCell.getX(), DirectedCoordinate.NORTH));
                    break;
                case 'D':
                    int minusX = 4;
                    while (curCell.getX() - minusX < 1 && minusX >= 3)
                        minusX++;
                    coorList.add(new DirectedCoordinate(curCell.getY(), curCell.getX() - minusX, DirectedCoordinate.EAST));
                    break;
            }
        }
        return coorList;

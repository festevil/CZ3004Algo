...
public class Robot {
	...
	*/
	 ...
	 * @param isRealRun real or virtual robot
	 */
	public Robot(Coordinate startPos, int startDir, boolean isRealRun, int padding) {
		this.isRealRun = isRealRun;
		this.padding = padding;

		int startY = startPos.getY() + padding;
		int startX = startPos.getX() + padding;
		boolean error = false;

		if (startY == 0 || startY == Map.maxY - 1 + 2 * padding)
			error = true;
		else if (startX == 0 || startX == Map.maxX - 1 + 2 * padding)
			error = true;
		...
		if (error == true) {
			...
			curPos = new Coordinate(1 + padding, 1 + padding);
			curDir = EAST;
		} 
		else {
			this.curPos = new Coordinate(startY, startX);
	...
	*/
	public Robot(Coordinate startPos, int startDir, boolean isRealRun) {
		this(startPos, startDir, isRealRun, 0);
	}
	...
	public void moveForward(int steps) {
		...
		switch (curDir) {
			case NORTH:
				...
				if (newPos < Map.maxY - 1 + 2 * padding) {
					curPos.setY(newPos);
				} 
				else
			...
			case EAST:
				...
				// Prevents Robot from going out of map boundary
				if (newPos < Map.maxX - 1 + 2 * padding) {
					curPos.setX(newPos);
				} 
				else
					System.out.println(warning);

	...
	public FastestPath(Map currMap, Coordinate startCoord, Coordinate goalCoord) {
		...
		for (int y = 1; y <= Map.maxY - 2; y++) {
			for (int x = 1; x <= Map.maxX - 2; x++) {
				...
				Robot robot = new Robot(currPos, Robot.EAST, false);
				...
				for (int i = 0; i < robotFootprint.length; i++) {
					...
					if (cell.getCellType() == Cell.WALL || cell.getCellType() == Cell.NORTHWALL ||
							cell.getCellType() == Cell.EASTWALL || cell.getCellType() == Cell.SOUTHWALL ||
							cell.getCellType() == Cell.WESTWALL) {
						hasWall = true;
						break;
					}
				}
